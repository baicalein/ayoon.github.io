...
**Data Science and Application of Artificial Intellegence in Medicine**
...
**PA LLM: Large Language Model for Prior Authorization Review, Adalimumab for Rheumatoid Arthritis (_2025 - Present_)** [HuggingFace Link](https://huggingface.co/baicalein/paLLM_adalimumab_RA_Lora)
- Develope a domain-specific Large Language Model (LLM) to automate prior authorization (PA) reviews for adalimumab in rheumatoid arthritis (RA) cases, based on FDA-approved indications and payer-specific clinical criteria. Used synthetic patient summaries and structured outputs to fine-tune a lightweight Llama-3.2–1B-Instruct model with LoRA for efficiency. Designed an instruction–response schema aligned with pharmacy workflow, evaluated outputs using GPT-5 as a reference "LLM-as-a-judge," and benchmarked model generalizability on MedMCQA, MMLU, and RACE datasets. The model aims to support pharmacists by reducing PA review burden while maintaining clinical accuracy and policy compliance.

**Melanoma Detection using Knowledge Distillation and Mobile Phone** [GitHub Link](https://github.com/rah-ds/Deep_Learning_Final_Project/tree/main) (_2025 - Present_)
- This deep learning project explores knowledge distillation (KD) to develop an efficient, lightweight melanoma detection model suitable for deployment on mobile devices. Using the HAM10000 dataset, we trained a compact student model (MobileNetV3-Small) guided by a larger teacher model (ResNet50) under various KD settings. We experimented with pooling strategies, focal loss, and KD hyperparameters (temperature and mixing ratio) to improve classification accuracy and sensitivity, especially for rare melanoma cases. Our work emphasizes explainability, edge-device compatibility, and precision at high sensitivity to support real-world, offline skin cancer screening.
